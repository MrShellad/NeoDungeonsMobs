...
public class GeomancyHelper {
	...
	public static final int[][] CONFIG_1_ROWS = new int[][] { CONFIG_1_NORTH_ROW, CONFIG_1_EAST_ROW,
			CONFIG_1_SOUTH_ROW, CONFIG_1_WEST_ROW };
	...
	public static final int[][] CONFIG_2_ROWS = new int[][] { CONFIG_2_NORTH_ROW, CONFIG_2_EAST_ROW,
			CONFIG_2_SOUTH_ROW, CONFIG_2_WEST_ROW };
	...
	private static BlockPos createCenteredBlockPosOnTarget(Entity targetEntity) {
		return new BlockPos(
				(int) Math.floor(targetEntity.getX()),
				(int) Math.floor(targetEntity.getY()),
				(int) Math.floor(targetEntity.getZ()));
	}

	private static void summonAreaDenialConstruct(LivingEntity casterEntity, Entity targetEntity,
			EntityType<? extends ConstructEntity> wallEntityType, double xshift, double zshift,
			Direction pillarFacing) {
		BlockPos targetPos = createCenteredBlockPosOnTarget(targetEntity).offset((int) xshift, 0,
				(int) zshift);
		boolean flag = false;
		double yShift = 0.0D;
		...
		do {
			...
			if (blockstate.isFaceSturdy(casterEntity.level(), blockpos1, Direction.UP)) {
				if (!casterEntity.level().isEmptyBlock(targetPos)) {
					...
					VoxelShape voxelshape = blockstate1.getCollisionShape(casterEntity.level(),
							targetPos);
	...
	private static void summonAreaDenialConstruct(LivingEntity casterEntity, BlockPos targetPos,
			EntityType<? extends ConstructEntity> wallEntityType, double xshift, double zshift,
			Direction pillarFacing) {
		targetPos = targetPos.offset((int) xshift, 0, (int) zshift);
		boolean flag = false;
		double yShift = 0.0D;
	...
	public static void summonOffensiveVine(LivingEntity casterEntity, LivingEntity targetEntity,
			EntityType<? extends AbstractVineEntity> entityType, int xShift, int zShift) {
		...
		if (canAllowBlockEntitySpawn(casterEntity, targetPos)) {
			...
			if (vineEntity != null) {
				vineEntity.setPos(targetPos.getX() + xShift, targetPos.getY(),
						targetPos.getZ() + zShift);
				...
				vineEntity.setLengthInBlocks(
						(float) vineLength + 3 + vineEntity.getRandom().nextInt(6));
	...
	private static void summonAreaDenialVine(LivingEntity casterEntity, LivingEntity targetEntity,
			EntityType<? extends AbstractVineEntity> entityType, double xshift, double zshift,
			Direction pillarFacing) {
		BlockPos targetPos = createCenteredBlockPosOnTarget(targetEntity).offset((int) xshift, 0, (int) zshift);
		// verify that the construct will be summoned on valid ground
		if (canAllowBlockEntitySpawn(casterEntity, targetPos)) {
			AbstractVineEntity vineEntity = entityType.create(casterEntity.level());
			if (vineEntity != null) {
	...
	public static void summonOffensiveConstruct(LivingEntity casterEntity, Entity targetEntity,
			EntityType<? extends ConstructEntity> wallEntityType, double xshift, double zshift,
			Direction pillarFacing) {
		BlockPos targetPos = createCenteredBlockPosOnTarget(targetEntity).offset((int) xshift, 0, (int) zshift);
		boolean flag = false;
	...
	public static void summonOffensiveConstruct(LivingEntity casterEntity, BlockPos targetPos,
			EntityType<? extends ConstructEntity> wallEntityType, double xshift, double zshift,
			Direction pillarFacing) {
		targetPos = targetPos.offset((int) xshift, 0, (int) zshift);
		boolean flag = false;
	...
	public static void summonAreaDenialTrap(LivingEntity casterEntity, Entity targetEntity,
			EntityType<? extends ConstructEntity> entityType, int[] rowToRemove) {
	...
	public static void summonAreaDenialTrap(LivingEntity casterEntity, BlockPos targetPos,
			EntityType<? extends ConstructEntity> entityType, int[] rowToRemove) {
	...
	public static void summonAreaDenialVineTrap(LivingEntity casterEntity, LivingEntity targetEntity,
			EntityType<? extends AbstractVineEntity> entityType, int[] rowToRemove) {
	...
	public static void summonWallTrap(LivingEntity casterEntity, BlockPos targetPos,
			EntityType<? extends ConstructEntity> entityType) {

		if (casterEntity.getRandom().nextBoolean()) {
			if (casterEntity.getRandom().nextBoolean()) {
				for (int length = -7; length < 7; length++) {
					summonAreaDenialConstruct(casterEntity, targetPos, entityType, -3, length,
							Direction.NORTH);
				}
			} else {
				for (int length = -7; length < 7; length++) {
			...
			if (casterEntity.getRandom().nextBoolean()) {
				for (int length = -7; length < 7; length++) {
					summonAreaDenialConstruct(casterEntity, targetPos, entityType, length, -3,
							Direction.NORTH);
				}
			} else {
				for (int length = -7; length < 7; length++) {
	...
	public static void summonWallTrap(LivingEntity casterEntity, Entity targetEntity,
			EntityType<? extends ConstructEntity> entityType) {
		BlockPos targetPos = createCenteredBlockPosOnTarget(targetEntity);
	...
	public static void summonRandomPillarsTrap(LivingEntity casterEntity, BlockPos targetPos,
			EntityType<? extends ConstructEntity> entityType) {

		for (int length = 0; length < 25; length++) {
			summonAreaDenialConstruct(casterEntity, targetPos, entityType,
					-8 + casterEntity.getRandom().nextInt(16),
					-8 + casterEntity.getRandom().nextInt(16), Direction.NORTH);
		}

	}

	public static void summonRandomPillarsTrap(LivingEntity casterEntity, Entity targetEntity,
			EntityType<? extends ConstructEntity> entityType) {
		BlockPos targetPos = createCenteredBlockPosOnTarget(targetEntity);
	...
	public static void summonQuadOffensiveTrap(LivingEntity casterEntity, BlockPos targetPos,
			EntityType<? extends ConstructEntity> entityType) {
	...
	public static void summonQuadOffensiveTrap(LivingEntity casterEntity, Entity targetEntity,
			EntityType<? extends ConstructEntity> entityType) {
		BlockPos targetPos = createCenteredBlockPosOnTarget(targetEntity);
		summonOffensiveConstruct(casterEntity, targetPos, entityType, -2, 0, Direction.NORTH);
		summonOffensiveConstruct(casterEntity, targetPos, entityType, 2, 0, Direction.NORTH);
