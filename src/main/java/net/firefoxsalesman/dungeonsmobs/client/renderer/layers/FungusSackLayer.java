...
public class FungusSackLayer<T extends LivingEntity, M extends EntityModel<T>> extends RenderLayer<T, M> {
	private static final ResourceLocation SACK_LOCATION = GeneralHelper.modLoc("textures/entity/fungus_sack.png");
	...
	public void render(PoseStack matrixStack, MultiBufferSource renderTypeBuffer, int p_225628_3_, T wearer,
		...
		if (shouldRender(itemstack, wearer)) {
			...
			getParentModel().copyPropertiesTo(fungusSackModel);
			fungusSackModel.setupAnim(wearer, p_225628_5_, p_225628_6_, p_225628_8_, p_225628_9_,
					p_225628_10_);
			VertexConsumer ivertexbuilder = ItemRenderer.getArmorFoilBuffer(renderTypeBuffer,
					RenderType.armorCutoutNoCull(resourcelocation), false);
			fungusSackModel.renderToBuffer(matrixStack, ivertexbuilder, p_225628_3_,
