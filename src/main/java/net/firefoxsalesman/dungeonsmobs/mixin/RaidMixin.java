...
import net.firefoxsalesman.dungeonsmobs.worldgen.BiomeSpecificRaider;
...
public class RaidMixin {
	...
	private Raider spawnNextWave(Raider abstractRaiderEntity, BlockPos blockPos) {
		...
		for (BiomeSpecificRaider currentBiomeSpecificRaider : BiomeSpecificRaider.values()) {
			...
			EntityType<?> originalRaiderType = abstractRaiderEntity != null ? abstractRaiderEntity.getType()
					: null;
			// the current iterated biome specific raider type
			EntityType<? extends Raider> currentBiomeSpecificRaiderType = currentBiomeSpecificRaider
					.getType();
			...
			if (!biomeTypeMatch
				...
				// replace the biome specific raider type to spawn with its replacement and
				// return it
				abstractRaiderEntity = equivalentType.create(this.level);
				return abstractRaiderEntity;
			}

			// check if the biome specific raider is contained in the raid wave member
			// values
			// in order to determine if the raider type to spawn should be replaced with the
			// current iterated biome specific raider type
			boolean raiderFoundInWaveMembers = false;
			for (Raid.RaiderType waveMember : Raid.RaiderType.values()) {
				EntityType<? extends Raider> waveMemberType = ((RaiderTypeAccessor) (Object) waveMember).dungeonsmobs$getEntityType();
			...
			// if the raider type to spawn is a type that gets replaced by the biome
			// specific raider's type
			// if the biome speciic raider that replaces it is not already contained in raid
			// wave member values
			if (biomeTypeMatch
				...
				// replace the raider type to spawn with its biome specific replacement and
				// return it
				abstractRaiderEntity = currentBiomeSpecificRaiderType.create(this.level);
				return abstractRaiderEntity;
			}
		}
		// if no replacements were determined to be made, allow the original raider type
		// to be spawned
		return abstractRaiderEntity;
	}
}
