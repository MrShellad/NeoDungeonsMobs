...
public class SlimeballEntity extends AbstractHurtingProjectile implements ItemSupplier {
	...
	public SlimeballEntity(Level worldIn) {
		super(ModEntities.SLIMEBALL.get(), worldIn);
	}
	...
	public void setStack(ItemStack stack) {
		if (!stack.is(Items.SLIME_BALL) || !stack.isComponentsPatchEmpty()) {
	...
	public void readAdditionalSaveData(CompoundTag compound) {
		...
		ItemStack itemstack = ItemStack.parse(this.registryAccess(), compound.getCompound("Item")).orElse(ItemStack.EMPTY);
	...
	protected void onHitEntity(EntityHitResult rayTraceResult) {
		...
		if (!(entity instanceof Slime)) {
			entity.hurt(damageSources().thrown(this, this.getOwner()), (float) attackDamage);
		}
	}
