package net.firefoxsalesman.dungeonsmobs.entity.slime;
...
public class ConjuredSlimeEntity extends Slime implements RangedAttackMob {
	...
	public ConjuredSlimeEntity(Level worldIn) {
		super(ModEntities.CONJURED_SLIME.get(), worldIn);
	}
	...
	public EntityType<? extends ConjuredSlimeEntity> getType() {
		return ModEntities.CONJURED_SLIME.get();
	}
	...
	public void performRangedAttack(LivingEntity target, float distanceFactor) {
		...
		float euclidDist = Mth.sqrt((float) (xDifference * xDifference + yDifference * yDifference
	...
	class ImmobileRangedAttackGoal extends Goal {
		...
		public boolean canUse() {
			...
			if (livingentity == null) {
				...
				return (!(livingentity instanceof Player)
						...
						&& ConjuredSlimeEntity.this
								.getMoveControl() instanceof MoveHelperController;
			}
		}
		...
		public void tick() {
			...
			if (attackTarget != null) {
				...
				// HANDLE SEE TIME
				double distanceSqToTarget = ConjuredSlimeEntity.this.distanceToSqr(attackTarget.getX(),
						attackTarget.getY(), attackTarget.getZ());
				boolean canSeeTarget = ConjuredSlimeEntity.this.getSensing()
						.hasLineOfSight(attackTarget);
				if (canSeeTarget) {
					++this.seeTime;
				} else {
					this.seeTime = 0;
				...
				if (--this.rangedAttackTime == 0) {
					...
					this.rangedAttackTime = Mth.floor(f
							* (float) (this.maxRangedAttackTime
									- this.minRangedAttackInterval)
							+ (float) this.minRangedAttackInterval);
				} else if (this.rangedAttackTime < 0) {
					float f2 = Mth.sqrt((float) distanceSqToTarget) / this.attackRadius;
					this.rangedAttackTime = Mth.floor(f2
							* (float) (this.maxRangedAttackTime
									- this.minRangedAttackInterval)
							+ (float) this.minRangedAttackInterval);
				}
