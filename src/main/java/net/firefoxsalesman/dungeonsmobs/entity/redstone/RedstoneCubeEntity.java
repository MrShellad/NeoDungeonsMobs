...
public class RedstoneCubeEntity extends Monster implements KeyframeEntity {
	...
	private static final EntityDataAccessor<Boolean> IS_ROLLING = SynchedEntityData
			.defineId(RedstoneCubeEntity.class, EntityDataSerializers.BOOLEAN);

	public RedstoneCubeEntity(Level worldIn) {
		super(ModEntities.REDSTONE_CUBE.get(), worldIn);
	}
	...
	protected void registerGoals() {
		...
		this.targetSelector.addGoal(1, new NearestAttackableTargetGoal<>(this, Player.class, 10, true, false,
				(entity) -> Math.abs(entity.getY() - this.getY()) <= 4.0D));
	...
	protected void dealDamage(LivingEntity entityIn) {
		if (this.isAlive()) {
			...
			if (this.distanceToSqr(entityIn) < 0.6D * (double) i * 0.6D * (double) i
					&& this.hasLineOfSight(entityIn)
					&& entityIn.hurt(damageSources().mobAttack(this),
	...
	static class AttackGoal extends Goal {
		...
		public void tick() {
			...
			((RedstoneCubeEntity.MoveHelperController) this.redstoneCubeEntity.getMoveControl())
					.setDirection(this.redstoneCubeEntity.getYRot(),
							this.redstoneCubeEntity.canDamagePlayer());
	...
	static class FaceRandomGoal extends Goal {
		 ...
		 */
		public boolean canUse() {
			return this.redstoneCubeEntity.getTarget() == null && (this.redstoneCubeEntity.onGround()
					|| this.redstoneCubeEntity.isInWater() || this.redstoneCubeEntity.isInLava()
					|| this.redstoneCubeEntity.hasEffect(MobEffects.LEVITATION));
		}
		...
		public void tick() {
			...
			if (this.operation == Operation.WAIT) {
				...
				if (this.mob.onGround() || this.mob.isInWater()) {
					...
					if (!this.redstoneCubeEntity.isRolling()
							&& this.redstoneCubeEntity.shouldRoll()) {
